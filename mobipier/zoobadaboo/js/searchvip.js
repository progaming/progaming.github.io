(function(){
	var app = angular.module('searchvip', []);
	app.controller("searchvipController", function($scope){
		//set Start
		var searchCtrl = this;
		...
		searchCtrl.vipInfoArr = new Array();
				  ...
				  success: function(objects) {
					...
					for(var i = 0; i < objects.length; i++)
					{
						...
						if(objects[i].has("active") && _active == true)
						{
							...
							{
								if(searchCtrl.vipIdArr.indexOf(uid) == -1)
								{
									searchCtrl.vipIdArr.push(uid);
									searchCtrl.vipInfoArr.push({"uid":uid,
																"sms":"X",
																"visa":"",
																"tel":tel});
								}
								...
								{
									for(var j = 0; j < searchCtrl.vipInfoArr.length; j++)
									{
										if(searchCtrl.vipInfoArr["uid"] == uid)
										{
											searchCtrl.vipInfoArr["sms"] = "X";
											if(tel != null && tel != "" && tel != undefined)
												searchCtrl.vipInfoArr["tel"] = tel;
										}
									}
								}
		...
		function GetCount()
		{
			searchCtrl.total = searchCtrl.vipIdArr.length;
			FindAllUser();
		}
		...
		function FindAllUser(){
			console.log("FindAllUser");
		 	...
		 	query.descending("updatedAt").containedIn("uid", searchCtrl.vipIdArr);
		 	...
		 	query.find({
				  success: function(objects) {
					...
					for(var i = 0; i < objects.length; i++)
					{
						...
						for(var j = 0; j < searchCtrl.vipInfoArr.length; j++)
						{
							if(searchCtrl.vipInfoArr[j]["uid"] == _uid)
							{
								sms = searchCtrl.vipInfoArr[j]["sms"];
								visa = searchCtrl.vipInfoArr[j]["visa"];
								tel = searchCtrl.vipInfoArr[j]["tel"];
								break;
							}
						}
